  ...
  return (
    <header className="sticky top-0 z-50 h-14 border-b border-zinc-800 bg-zinc-950/90 backdrop-blur">
      <div className="flex h-full items-center justify-between gap-3 px-4">
        ...
        {title ? <span className="truncate font-mono text-[10px] uppercase tracking-[0.32em] text-zinc-500">{title}</span> : <span />}
        <span className="max-w-35 truncate rounded-md bg-zinc-800 px-2 py-1 font-mono text-xs text-zinc-300">{truncatedAddress}</span>
      </div>
    </header>
  );
}
